...
In order to bypass the options asked during install, set the values in bgeespawn/ay_bgeespawn.ini and copy it to the override folder.
...
HISTORY
Version 0.7: Tweaked the range checks, add automatic INI preset options, add an option to keep the vanilla spawnpoints enabled (to allow it plaiyng nicely with the BG Enemy Randomizer mod), add Project Infinity metadata (by Graion Dilach)

Version 0.6: Added EasyTuTu-styled spawning option (by MachoGrande)

Version 0.5: Attempt increasing the range checks, added death variables, WeiDU update (by solar_one)
